...
CREATE TABLE Speaker (
    ...
    PreviousSpeakingEngagements VARCHAR(255),
    EventID INT,
    FOREIGN KEY (EventID) REFERENCES Event(EventID)
);
CREATE TABLE Venue (
...
CREATE TABLE Schedule (
    ...
    Date DATE,
    Time TIME,
    ActivitySessionName VARCHAR(255),
    SpeakerID INT,
    VenueID INT,
    FOREIGN KEY (EventID) REFERENCES Event(EventID),
...
ALTER table Event add column (FOREIGN KEY (Organizer_ID) REFERENCES Organizer(OrganizerID));
...
SELECT Attendee.FirstName, Attendee.LastName, Event.EventName
FROM Attendee
JOIN Event ON Attendee.EventID = Event.EventID;

SELECT Speaker.SpeakerName, Event.EventName
FROM Speaker
JOIN Event ON Speaker.EventID = Event.EventID;

SELECT Venue.VenueName, Event.EventName
FROM Venue
JOIN Event ON Venue.VenueID = Event.EventID;

SELECT Ticket.TicketType, Ticket.Price, Attendee.FirstName, Attendee.LastName
FROM Ticket
JOIN Attendee ON Ticket.AttendeeID = Attendee.AttendeeID;

SELECT Schedule.Date, Schedule.Time, Schedule.ActivitySessionName, Event.EventName, Venue.VenueName
FROM Schedule
JOIN Event ON Schedule.EventID = Event.EventID
JOIN Venue ON Schedule.VenueID = Venue.VenueID;

SELECT Feedback.Rating, Feedback.Comments, Event.EventName, Attendee.FirstName, Attendee.LastName
FROM Feedback
JOIN Event ON Feedback.EventID = Event.EventID
JOIN Attendee ON Feedback.AttendeeID = Attendee.AttendeeID;

INSERT INTO Event (EventID, EventName, EventDescription, StartDate, EndDate, Location, Organizer_ID, Category, RegistrationDeadline, MaximumCapacity, CurrentAttendance, Status, Speaker_ID)
VALUES (11, 'Technology Conference', 'Annual conference on technology', '2024-10-20', '2024-10-22', 'Convention Center', 5, 'Technology', '2024-10-01', 400, 200, 'Active', 5);

UPDATE Event
SET Status = 'Cancelled'
WHERE EventID = 11;

SELECT Volunteer.Name, VolunteerTasks.AssignedTask
FROM Volunteer
JOIN VolunteerTasks ON Volunteer.VolunteerID = VolunteerTasks.VolunteerID;